...
import { CITIES, Currency } from '../constants.js';
...
const getMockWeatherData = (lat, language) => {
...
const getFallbackExchangeRate = (from, to) => {
    const usdToArs = 900;
    const usdToEur = 0.93;
    const usdToIls = 3.72;
    const rates = {
...
export async function askGemini(userPrompt, language) {
  if (!isAiAvailable()) {
...
export async function sendMessageInChat(systemInstruction, history, newMessage, language) {
    if (!isAiAvailable()) {
...
export async function translateText(textToTranslate, language) {
    if (!isAiAvailable()) {
...
export async function findEventsWithGoogleSearch(prompt, language) {
    if (!isAiAvailable()) {
...
export async function getCachedExchangeRate(from, to) {
  if (from === to) return 1;
...
export async function getWeatherForecast(coords, language) {
