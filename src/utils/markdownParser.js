  ...
  while ((match = linkRegex.exec(text)) !== null) {
    ...
    parts.push(
      React.createElement('a', {
        ...
        className: 'text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 underline transition-colors duration-150'
      }, match[1])
    );
...
export const parseMarkdownTable = (
  ...
  const sanitizeCell = (cellText) => cellText.trim();
  ...
  const columnAlignments = headers.map(hKey => {
  ...
  return (
    React.createElement('div', { className: "overflow-x-auto my-4" },
      React.createElement('table', { className: "min-w-full divide-y divide-gray-300 dark:divide-slate-600 border border-gray-300 dark:border-slate-600 rounded-lg shadow-sm" },
        React.createElement('thead', { className: "bg-indigo-50 dark:bg-slate-700" },
          React.createElement('tr', null,
            headers.map((header, index) => (
              React.createElement('th', {
                key: index,
                scope: "col",
                className: `px-4 py-3 ${(columnAlignments[index] || (lang === Language.HE ? 'text-right' : 'text-left'))} text-sm font-semibold text-indigo-700 dark:text-indigo-300`
              }, header)
            ))
          )
        ),
        React.createElement('tbody', { className: "divide-y divide-gray-200 dark:divide-slate-600 bg-white dark:bg-slate-800" },
          bodyRows.map((row, rowIndex) => (
            React.createElement('tr', { key: rowIndex, className: (rowIndex % 2 === 0 ? undefined : 'bg-gray-50 dark:bg-slate-700/50') },
              row.map((cell, cellIndex) => {
                ...
                return (
                  React.createElement('td', {
                    key: cellIndex,
                    className: `px-4 py-3 whitespace-nowrap text-sm text-gray-600 dark:text-slate-400 ${(columnAlignments[cellIndex] || (lang === Language.HE ? 'text-right' : 'text-left'))}`
                  }, cellContent)
                );
              })
            )
